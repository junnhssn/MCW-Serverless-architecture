...
3. On the **Create storage account** blade, specify the following configuration options:
    ...
    -  **Name**: Enter a unique value for the storage account name, such as **tollboothstorageDID** where DID is unique ID present in your Azure username. For example: Username-<inject key="AzureAdUserEmail"></inject>, here six digit number is the **DeploymentID**. 
