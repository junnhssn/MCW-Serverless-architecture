...
1. After the Application Insights account has completed provisioning, open the instance by opening the **serverless-architecture-DID** resource group, and then selecting the your recently created application insights instance.
...
2. Select **Application Insights** from the left hand menu, under **Application Insights** select **Select existing resource** then select the application insights which you created previously and click on **ok**. 

    > **Note**: It may take a few minutes for this link to display. Try refreshing the page after a few moments.

    ![In the TollBoothFunctionApp blade, under Configured features, the Application Insights link is selected.](media/image006.png 'TollBoothFunctionApp blade')

3. Open the Azure Function App you created whose name ends with **Events**, or the name you specified for the Function App containing the NodeJS functions.

4. Select **Application Insights** from the left hand menu, under **Application Insights** select **Select existing resource** then select the application insights which you created previously and click on **ok**. 

    ![In the TollBootheventsFunctionApp blade, under Configured features, the Application Insights link is selected.](media/image009.png 'TollBoothFunctionApp blade')

5. On the Application Insights pane of function app, scroll down and select **VIEW MORE IN APPLICATION INSIGHTS**.

    ![In the TollBootheventsFunctionApp blade, under Configured features, the Application Insights link is selected.](media/image008.png 'TollBoothFunctionApp blade')

6. In Application Insights, select **Live Metrics Stream** underneath Investigate in the menu.
...
7. Leave the Live Metrics Stream open and go back to the starter app solution in Visual Studio.

8. Navigate to the **UploadImages** project using the Solution Explorer of Visual Studio. Right-click on **UploadImages**, then select **Properties**.
...
11. Right-click the **UploadImages** project in the Solution Explorer, then select **Debug** then **Start new instance** from the context menu.
